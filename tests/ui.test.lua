...
-- `bridge.*` belongs to the SSH remote bridge (FX.bridgeSync, bridge.intervalSeconds). The
-- tab bridge first read `bridge.enabled` too, so a config with the remote bridge off (its
-- default) switched Close-by-tab off, and the example config grew a second "bridge" key.
do
  local f = io.open(ROOT .. "claude-dashboard.lua", "r")
  local src = f and f:read("*a") or ""
  if f then f:close() end
  local e = io.open(ROOT .. "cc-config.example.json", "r")
  local ex = e and e:read("*a") or ""
  ...
  check("tabbridge-pin: Close-by-tab and Doctor read tabBridge.enabled, never the remote bridge's key",
        ...
        and closeTab:find('"tabBridge.enabled"', 1, true) ~= nil and closeTab:find('"bridge.enabled"', 1, true) == nil
